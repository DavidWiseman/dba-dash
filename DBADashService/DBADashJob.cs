    ...
    public class DBADashJob : IJob
    {
        ...
        private static async Task CollectS3(DBADashSource cfg)
        {
            ...
            try
            {
                ...
                ListObjectsRequest request = new() { BucketName = bucket, Prefix = (keyPrefix + "/DBADash_").Replace("//", "/") };
                ...
                do
                {
                    ...
                    if (resp is { S3Objects: not null })
                    {
                        ...
                        Log.Information("Processing {0} files from {1}. Instance Count: {2}", resp.S3Objects.Count, keyPrefix, filesToProcessByInstance.Count);
