...
namespace DBADashService
{
    ...
    public class WorkItem
    {
        ...
        public CollectionType[] Types { get; set; }
        public Dictionary<string, CustomCollection> CustomCollections { get; set; }
        public WorkItemState State { get; set; }
        public DateTime? PreviousFireTime { get; set; }
        public string Schedule { get; set; } // Track which schedule this work item belongs to

        private DateTime EnqueueTime = DateTime.Now;
        ...
        public string DedupKey => $"{Source.ConnectionID ?? Source.ConnectionString}::{Schedule}::{Types}{string.Join('|', CustomCollections.Keys.OrderBy(k => k))}";

        public async Task ExecuteAsync(CollectionConfig config, CancellationToken cancellationToken)
        {
            if (Types == null || Types.Length == 0)
                return;
            var dequeueLatencyMs = DateTime.Now.Subtract(EnqueueTime).TotalMilliseconds;
